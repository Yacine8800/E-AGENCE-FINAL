...
const Navbar = () => {
  ...
  return (
    <>
      {/* Navbar container */}
      <div className="px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between mt-6 relative z-[100] ">
        {/* Left side */}
        <div className="flex items-center gap-2 sm:gap-8">
          {/* Burger button (mobile only) */}
          <button
            onClick={toggleSidebar}
            className="lg:hidden w-[40px] sm:w-[50px] h-[40px] sm:h-[50px] bg-white border border-gray-200 shadow-sm rounded-xl flex items-center justify-center cursor-pointer hover:bg-gray-50 hover:shadow transition-all duration-200 transform hover:-translate-y-0.5"
            aria-label={isSidebarOpen ? "Close menu" : "Open menu"}
          >
            {isSidebarOpen ? (
              <X size={20} className="text-gray-700 sm:h-5 sm:w-5" />
            ) : (
              <Menu size={20} className="text-gray-700 sm:h-5 sm:w-5" />
            )}
          </button>
          <Link
            href="/"
      ...
      {previewImage && (
        <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-[10001] p-4 fade-in">
          <div className="relative max-w-5xl w-full h-full flex items-center justify-center">
            ...
            {!isZoomed && (
              <div className="fixed inset-0 w-full bg-black/60 backdrop-blur-sm transition-opacity duration-300 z-[-1]">
                {/* Bouton close modal */}
                <button
                  onClick={() =>
                    setSelectedNotification((prev) => ({ ...prev, isOpen: false }))
                  }
                  className="text-gray-500 hover:text-gray-700 absolute right-6 top-6 p-2 rounded-full hover:bg-gray-100 transition-all duration-200"
                  title="Fermer"
                >
                  <svg
                    className="w-6 h-6"
