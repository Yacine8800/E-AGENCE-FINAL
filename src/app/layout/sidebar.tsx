...
const Sidebar = () => {
  ...
  const [profileImage, setProfileImage] = useState<string>('/assets/personne.png');
  ...
  useEffect(() => {
    ...
    if (status.profileImageValue) {
      setProfileImage(status.profileImageValue);
    }
    // Mettre à jour le nom et prénom
    setUserName({
      firstname: status.firstnameValue || user?.firstname,
      lastname: status.lastnameValue || user?.lastname
    });
  }, [user]);
  ...
  useEffect(() => {
    ...
    // Vérifier aussi les changements locaux
    const checkInterval = setInterval(handleStorageChange, 100);
  ...
  const handleAccountEditSubmit = (data: {
    phone?: string;
    email?: string;
    idFile?: string;
  }) => {
  ...
  // Gestion du changement de mode du modal
  const handleModalModeChange = (newMode: "all" | "phone" | "email" | "id") => {
          ...
          <div className="mb-4 bg-white rounded-[40px] lg:rounded-2xl p-3 lg:mb-6 pb-4 lg:pb-6 border-b border-gray-200/30 z-10 relative">
            <div className="flex flex-col items-center">
              ...
              <p className="text-xs font-medium text-noir mb-1 flex items-center gap-1">
                {mounted
                  ? (isEmailVerified ? savedValues.email : user?.email) ||
              ...
              <p className="text-xs font-medium text-noir flex items-center gap-1">
                {mounted
                  ? (isPhoneVerified ? savedValues.phone : user?.contact) ||
