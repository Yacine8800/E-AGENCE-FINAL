...
interface AccountEditSheetProps {
  ...
  onSubmit: (data: { phone?: string; email?: string; idFile?: string }) => void;
  phoneVerified?: boolean;
  emailVerified?: boolean;
  idVerified?: boolean;
}

const AccountEditSheet: React.FC<AccountEditSheetProps> = ({ open, onClose, mode, initialValues, onSubmit, phoneVerified, emailVerified, idVerified }) => {
  if (!open) return null;
  ...
  const handlePieceChange = (type: string, image: string | null) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      parsed = { ...parsed, idFileValue: image, idTypeValue: type };
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data: { phone?: string; email?: string; idFile?: string } = {};
    if (mode === "all" || mode === "phone") {
      data.phone = (form.elements.namedItem("phone") as HTMLInputElement)?.value;
    }
    if (mode === "all" || mode === "email") {
      data.email = (form.elements.namedItem("email") as HTMLInputElement)?.value;
    }
    if (mode === "all" || mode === "id") {
      // On utilise l'image base64 sauvegardée
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex justify-end">
      ...
      <div
        ...
        onClick={onClose}
      ></div>
      <div className="relative w-full md:w-[600px] bg-white h-full shadow-2xl overflow-auto flex flex-col rounded-l-2xl transition-all duration-500">
        {/* HEADER */}
          ...
          {(mode === "all" || mode === "id") && (
            <div className="w-full flex flex-col gap-2">
              <label htmlFor="idFile" className="text-sm font-semibold flex items-center gap-2">
                Pièce de l'utilisateur <span className="text-primary">*</span>
                {idVerified && (
                  <span className="flex items-center gap-1 text-green-600 text-xs font-normal">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path fillRule="evenodd" clipRule="evenodd" d="M8 15.5513C8.98491 15.5513 9.96018 15.3573 10.8701 14.9804C11.7801 14.6035 12.6069 14.051 13.3033 13.3546C13.9997 12.6581 14.5522 11.8313 14.9291 10.9214C15.306 10.0115 15.5 9.03618 15.5 8.05127C15.5 7.06636 15.306 6.09109 14.9291 5.18114C14.5522 4.2712 13.9997 3.44441 13.3033 2.74797C12.6069 2.05153 11.7801 1.49908 10.8701 1.12217C9.96018 0.745263 8.98491 0.55127 8 0.55127C6.01088 0.55127 4.10322 1.34145 2.6967 2.74797C1.29018 4.15449 0.5 6.06215 0.5 8.05127C0.5 10.0404 1.29018 11.948 2.6967 13.3546C4.10322 14.7611 6.01088 15.5513 8 15.5513ZM7.80667 11.0846L11.9733 6.0846L10.6933 5.01794L7.11 9.3171L5.25583 7.4621L4.0775 8.64044L6.5775 11.1404L7.2225 11.7854L7.80667 11.0846Z" fill="#2DAE9F"/>
              ...
              {/* Utilisation de l'input custom */}
              <InputTypePiece initialImage={pieceImage} initialType={pieceType} onChange={handlePieceChange} />
            </div>
          )}
          <button 
            type="submit" 
            className="bg-primary text-white rounded-lg px-4 py-3 font-semibold mt-4"
            disabled={isPieceIncomplete}
            style={isPieceIncomplete ? { opacity: 0.5, cursor: 'not-allowed' } : {}}
          >
            {mode === 'id' ? 'Enregistrer' : 'Vérifier'}
          </button>
        </form>
      </div>
    </div>
